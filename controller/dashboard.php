...
class dashboard extends fs_controller
{
    ...
    protected function private_core()
    {
        ...
        $this->mes = array(
            ...
            'impuestos_mejora' => 0,
            'impuestos_rent_fuente' => 0,
            'impuestos_rent_iva' => 0,
            'beneficios_rent' => 0,
            'beneficios' => 0,
    ...
    private function calcula_periodo(&$stats, $pendiente = FALSE)
    {
        ...
        {
            /// calculamos las rentciones_fuente de este mes
            $sql = "select sum(rent_fuente) as total_fuente from facturascli where"
            ...
            if($data)
            {
                $stats['impuestos_rent_fuente'] =  floatval($data[0]['total_fuente']) ;

            }

            /// calculamos las rentciones_iva de este mes
            $sql = "select sum(rent_iva) as total_iva from facturascli where"
                . " fecha >= ".$this->empresa->var2str($stats['desde'])
                . " and fecha <= ".$this->empresa->var2str($stats['hasta']).";";
            ...
            if($data)
            {
                $stats['impuestos_rent_iva'] =  floatval($data[0]['total_iva']) ;
        ...
        $stats['beneficios_rent'] = $stats['ventas'] - $stats['compras'] - $stats['impuestos'] - $stats['impuestos_rent_fuente'] - $stats['impuestos_rent_iva'];
